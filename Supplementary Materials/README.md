...
- **`variable_codebook.md`** — Lookup table for the short variable names
  ...
  `capb_agg`, `ida_irr`, `aps205`, `nssi_any`), plus a glossary of the
  statistical column headers (`omnibus_p`, `p_fdr`, ω², OR, `min_bch_weight`,
  etc.).
...
1. `technical_summary.md` for the methods.
2. `complete_results.md` for the numbers behind every result described in
   (1).
3. `closed_form_bch.R` if you want to see or run the actual statistical
   code, not just read about it.
4. `variable_codebook.md` as a reference, if needed, especially to look
    scoring guidelines.
5. `session_info.md` for exact package versions.
