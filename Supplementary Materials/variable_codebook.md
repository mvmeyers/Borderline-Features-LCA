...
|---|---|---|---|
| `capb_agg` | Aggression (CASI Category B) | 0–3 | Higher = more aggression |
...
|---|---|---|
| `nssi_any` | Any non-suicidal self-injury method endorsed | Binary; 11 YRBS-HS method items |
| `nssi_sum_wins` | Count of NSSI methods endorsed, winsorized | Capped at the 95th percentile (= 8) |
...
severity ranking — see `technical_summary.md`'s typology-vs-continuum
...
| `AIC` / `BIC` / `aBIC` / `CAIC` | Model fit indices used to compare different K (lower = better fit, with tradeoffs — see `technical_summary.md`) |
